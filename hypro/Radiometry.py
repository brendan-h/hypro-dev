...
def dn2rdn_Hyspex(rdn_image_file, dn_image_file, radio_cali_file, acquisition_time):
    ...
    for from_line in range(0, dn_header['lines'], 500):
        ...
        if radio_cali_header['bands']==2:
            ...
            background = np.stack([radio_cali_coeff[1,:,:]]*(to_line-from_line)) #b1
            backgroundLast = np.stack([radio_cali_coeff[2,:,:]]*(to_line-from_line)) #b2
            
            # For clipped DN images:
            if "clip tag" in dn_header.keys() and dn_header['clip tag']==1:
                    l_start = dn_header['starting line']-1
                    raw_lines = dn_header['original lines']
                    factor = np.arange(from_line+l_start, to_line+l_start)/raw_lines
        ...
        rdn = (dn_image[from_line:to_line,:,:].astype('float32')-offset)*gain*1000 # convert rdn to int16, by Ting    
        #rdn = (DN-b)*a
        
        # Write radiance to the file.
        # fid.write(rdn.astype('float32').tostring())
        fid.write(rdn.astype('int16').tostring())
    ...
    rdn_header['data type'] = 2  # int type, by Ting
    ...
    for from_line in range(0, raw_rdn_header['lines'], 500):
        ...
        for sample in range(raw_rdn_header['samples']):
            f = interpolate.interp1d(smile_effect_data[0,:,sample],
                                     raw_rdn_image[from_line:to_line,:,sample]/1000, # divided by 1000 to convert back to original rdn, by Ting
                                     kind='cubic',
                                     fill_value='extrapolate',
                                     axis=1)
            rdn[:,:,sample] = f(raw_rdn_header['wavelength'])
            rdn[:,:,sample] = rdn[:,:,sample]*1000  # times 1000 to convert to int, by Ting
            del f
