...
def average_rdn(avg_rdn_file, rdn_image_file, sca_image_file, pre_class_image_file):
    ...
    for band in range(rdn_header['bands']):
        ...
        # Average radiance.
        rdn = np.ma.array(rdn_image[:,band,:]/1000, mask=~tmp_mask).mean(axis=0) # shape = (samples,1) # divide by 1000 to convert back to original rdn, by Ting
