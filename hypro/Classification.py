...
def pre_classification(pre_class_image_file, rdn_image_file, sun_zenith, distance, background_mask_file=None):
    ...
    if if_whole:
        ...
        if rdn_header['interleave'].lower() == 'bil':
            ...
            blue_refl = rdn_image[:,blue_band,:]/1000*np.pi*d2/(solar_flux[blue_band]*cos_sun_zenith) # convert back to oringinal rdn by /1000. same below
            # green_refl = rdn_image[:,green_band,:]*np.pi*d2/(solar_flux[green_band]*cos_sun_zenith)
            green_refl = rdn_image[:,green_band,:]/1000*np.pi*d2/(solar_flux[green_band]*cos_sun_zenith)
            # red_refl = rdn_image[:,red_band,:]*np.pi*d2/(solar_flux[red_band]*cos_sun_zenith)
            red_refl = rdn_image[:,red_band,:]/1000*np.pi*d2/(solar_flux[red_band]*cos_sun_zenith)
            # nir_refl = rdn_image[:,nir_band,:]*np.pi*d2/(solar_flux[nir_band]*cos_sun_zenith)
            nir_refl = rdn_image[:,nir_band,:]/1000*np.pi*d2/(solar_flux[nir_band]*cos_sun_zenith)

            # cirrus_refl = rdn_image[:,cirrus_band,:]*np.pi*d2/(solar_flux[cirrus_band]*cos_sun_zenith)
            cirrus_refl = rdn_image[:,cirrus_band,:]/1000*np.pi*d2/(solar_flux[cirrus_band]*cos_sun_zenith)
            swir1_refl = rdn_image[:,swir1_band,:]/1000*np.pi*d2/(solar_flux[swir1_band]*cos_sun_zenith)
            swir2_refl = rdn_image[:,swir2_band,:]/1000*np.pi*d2/(solar_flux[swir2_band]*cos_sun_zenith)
        else:
            blue_refl = rdn_image[blue_band,:,:]/1000*np.pi*d2/(solar_flux[blue_band]*cos_sun_zenith)
            green_refl = rdn_image[green_band,:,:]/1000*np.pi*d2/(solar_flux[green_band]*cos_sun_zenith)
            red_refl = rdn_image[red_band,:,:]/1000*np.pi*d2/(solar_flux[red_band]*cos_sun_zenith)
            nir_refl = rdn_image[nir_band,:,:]/1000*np.pi*d2/(solar_flux[nir_band]*cos_sun_zenith)
        ...
        # Calculate the reflectance at different bands.
        if rdn_header['interleave'].lower() == 'bil':
            blue_refl = rdn_image[:,blue_band,:]/1000*np.pi*d2/(solar_flux[blue_band]*cos_sun_zenith) # divide rdn by 1000 to convert back to original, by Ting 
            green_refl = rdn_image[:,green_band,:]/1000*np.pi*d2/(solar_flux[green_band]*cos_sun_zenith)
            red_refl = rdn_image[:,red_band,:]/1000*np.pi*d2/(solar_flux[red_band]*cos_sun_zenith)
            nir_refl = rdn_image[:,nir_band,:]/1000*np.pi*d2/(solar_flux[nir_band]*cos_sun_zenith)
        else:
        ...
        if rdn_header['interleave'] == 'bil':
            cirrus_refl = rdn_image[:,cirrus_band,:]/1000*np.pi*d2/(solar_flux[cirrus_band]*cos_sun_zenith)# divide rdn by 1000 to convert back to original, by Ting 
            swir1_refl = rdn_image[:,swir1_band,:]/1000*np.pi*d2/(solar_flux[swir1_band]*cos_sun_zenith)
            swir2_refl = rdn_image[:,swir2_band,:]/1000*np.pi*d2/(solar_flux[swir2_band]*cos_sun_zenith)
        else:
            cirrus_refl = rdn_image[cirrus_band,:,:]/1000*np.pi*d2/(solar_flux[cirrus_band]*cos_sun_zenith)
