...
def estimate_wvc(wvc_file, rdn_file, sensors, sun_zenith, distance, background_mask_file):
    ...
    if abs(wave-470)<20:
        refl = rdn_image[band,:,:]/1000*np.pi*d2/(solar_flux[band]*cos_sun_zenith) # divided by 1000 to convert back to original rdn, Ting. same below
    ...
    if abs(wave-850)<20:
        refl = rdn_image[band,:,:]/1000*np.pi*d2/(solar_flux[band]*cos_sun_zenith) # dtype modify
    ...
    for sensor_index, sensor_dict in sensors.items():
        ...
        for wvc_model in wvc_models.values():
            ...
            # divided by 1000 to convert back to original rdn, by Ting
            ratio_image = (rdn_image[bands[1],:,:]/1000)/(1e-10+rdn_image[bands[0],:,:]/1000*wvc_model['weight'][0]+rdn_image[bands[2],:,:]/1000*wvc_model['weight'][1])
