...
(4) The input dataset of HyspexPro includes:
    ...
    Below is the standard workflow of HyspexPro:
                            Input dataset
                                 ...
                                 \|/
                                  V
                Clip the IMUGPS and raw DN image to user defined extent                           
                                  |
                                 \|/
                             Prepare IMU&GPS
                                  |
...
REFERENCES:
----------
[1] Gao B., et al. (2004). Remote Sens. Environ. 90(4): 424-433. doi:10.1016/j.rse.2003.09.002
[2] Kaufman Y., et al. (1992). IEEE Trans. Geosci. Remote Sens. 30(5): 871–884. doi:10.1109/36.175321
[3] Kaufman Y., et al. (1997). IEEE Trans. Geosci. Remote Sens. 35(5): 1286-1298. doi:10.1109/36.628795
...
from Clip_by_Extent   import clip_imugps,clip_hyspex
...
def initialize_flight_dict(config, flight_index):
    ...
    # shape file for the flight box:
    flight_dict['map_shp'] = config['map_shp']
...
def HyspexPro(config_file):
    ...
    for flight_index in flight_indices:
        ...
        for sensor_index, sensor_dict in flight_dict['sensors'].items():
            ...
            # Clip the data and update flight_dict/sensor_dict, note: the clipped data are stored under the main directory.           
            flight_log.info('Clip the IMUGPS and raw img to user defined extent.')
            sensor_dict['clipped_imugps_file'] = os.path.join(config['Data']['output_dir'], basename+'_ClippedIMUGPS.txt')
            # clip the imugps
            clip_tag = clip_imugps(sensor_dict['raw_imugps_file'],
                        flight_dict['map_shp'],
                        sensor_dict['clipped_imugps_file'])
            # If the flight line is outside of ROI:
            if clip_tag == 0:
                break
            
            sensor_dict['clipped_dn_file'] = os.path.join(config['Data']['output_dir'], basename+'_ClippedDN')
            # clip the raw dn image
            clip_hyspex(sensor_dict['dn_image_file'],
                        sensor_dict['clipped_dn_file'],
                        sensor_dict['clipped_imugps_file'])
        ...
        # If the flight line is outside of ROI, break out of the loop
        if clip_tag == 0:
            flight_log.info('Flight line outside of ROI, skip')
            continue
