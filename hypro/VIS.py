...
def estimate_vis(vis_file, ddv_file, atm_lut_file, rdn_file, sca_file, background_mask_file):
    ...
    red_refl = atm_corr_band(atm_lut_WVC, atm_lut_VIS, atm_lut_VZA, atm_lut_RAA, atm_lut[...,red_band],
                             tmp_wvc_image, tmp_vis_image, vza_image, raa_image, rdn_image[red_band,:,:]/1000, # divided by 1000 to convert to original rdn, by Ting
                             bg_mask)
    
    nir_refl = atm_corr_band(atm_lut_WVC, atm_lut_VIS, atm_lut_VZA, atm_lut_RAA, atm_lut[...,nir_band],
                             tmp_wvc_image, tmp_vis_image, vza_image, raa_image, rdn_image[nir_band,:,:]/1000,
                             bg_mask)
    ...
    if if_vnir and if_swir:
        ...
        swir_refl = atm_corr_band(atm_lut_WVC, atm_lut_VIS, atm_lut_VZA, atm_lut_RAA, atm_lut[...,swir_band],
                                  tmp_wvc_image, tmp_vis_image, vza_image, raa_image, rdn_image[swir_band,:,:]/1000, # divided by 1000 to convert to original rdn, by Ting
                                  bg_mask)
        ...
        for row, column in zip(rows, columns):
            ...
            vis_image[row, column] = np.interp(rdn_image[red_band,row,column]/1000, interp_rdn[::-1], atm_lut_VIS[::-1]) # divided by 1000 to convert to original rdn, by Ting
        rdn_image.flush()
        del rdn_image
