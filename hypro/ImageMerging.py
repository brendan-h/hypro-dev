...
def merge_dem_sca(background_mask_file, merged_dem_file, merged_sca_file, sensors):
    ...
    lrx, lry = np.floor(lrx/pixel_size-1)*pixel_size, np.ceil(lry/pixel_size+1)*pixel_size
...
def merge_rdn(merged_image_file, mask_file, sensors):
    ...
    for v in bands_waves_fwhms:
        ...
        if ((v[1]>=1339.0)&(v[1]<=1438.0))|((v[1]>=1808.0)&(v[1]<=1978.0))|(v[1]>=2467.0):
            ...
            resampled_image = np.zeros(x.shape,dtype ='int16') # in int, by Ting
        else:
            offset = header['header offset']+2*band*header['lines']*header['samples']# in bytes, int16 consists 2 bytes      
            ...
            resampled_image = resample_ortho_rdn(np.copy(rdn_image)/1000, # divided by 1000 to convert back to original rdn
        ...
        # fid.write(resampled_image.astype('float32').tostring())
        fid.write(resampled_image.astype('int16').tostring()) # write in int ,by Ting 
        del resampled_image
    fid.close()
